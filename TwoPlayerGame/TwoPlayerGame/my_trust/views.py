...
class SendBack(Page):
    ...
    def vars_for_template(self):
        if self.group.sent_amount == 1:
            self.session.vars['proposer_selection'] = self.session.vars['options'][0]
        else:
            self.session.vars['proposer_selection'] = self.session.vars['options'][1]
